...
function main()
    ...
    while is_run do
        ...
        if current_quote and (not last_quote or has_quote_changed(last_quote, current_quote)) then
            ...
            Update_Level_Table(l_id,t_id,current_quote)
...
function Update_Level_Table(table_Id,level_id,current_quote)
            ...
            Print_Values(table_Id,level_id,MyQuote,current_quote)
    ...
    function Print_Values(id,level_id,array,quote)
        if #array == 0 then
        Clear(id)
        return
        end
...
----------------------------------------------------------------------------------------------
        function SetStyle(level2, tableId, row, price, volume)
            if #level2.bid == 0 or #level2.offer == 0 then
            return
        end
