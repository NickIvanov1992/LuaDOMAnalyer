...
function Update_Level_Table(table_Id,current_quote)
        if #current_quote.bid + #current_quote.offer > #MyQuote.bids + #MyQuote.asks then
            message("пиздец")
        end
        for i = #current_quote.bid, 1, -1 do
            if(find_Element(MyQuote.bids.price,current_quote.bid[i].price) == false) then
                table.insert(MyQuote.bids[i].price, current_quote.bid[i].price)
            end
        end
    end
    ...
    function find_Element(array,element)
        value = false
        for j = 1, #MyQuote.bids.price, 1 do
            if (MyQuote.bids.price[j] == element) then
                value = true
                break
            end
        end
